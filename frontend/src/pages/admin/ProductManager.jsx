...
const ProductManager = () => {
  ...
  const uploadImage = async (file) => {
    ...
    return res.data.image; // server trả đường dẫn
  };
  ...
  const openAddModal = () => {
    setIsEdit(false);
    setPreviewFile(null);
    setFormData({ _id: "", name: "", slug: "", description: "", price: 0, salePrice: 0, category: "", images: [] });
    setShowModal(true);
  };
    ...
    <div className="p-6">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {products.length > 0 ? products.map((p) => (
                ...
                <tr><td colSpan="5" className="p-6 text-center">Chưa có sản phẩm</td></tr>
              )}
            </tbody>
          </table>
        )}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl w-full max-w-2xl p-6 overflow-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full border px-3 py-2 rounded" />
              </div>

              <div>
                <label className="block mb-1">Danh mục</label>
                <select required value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full border px-3 py-2 rounded">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" min="0" required value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} className="w-full border px-3 py-2 rounded" />
                </div>
                <div>
                  <label className="block mb-1">Giá khuyến mãi</label>
                  <input type="number" min="0" value={formData.salePrice} onChange={e => setFormData({...formData, salePrice: e.target.value})} className="w-full border px-3 py-2 rounded" />
                </div>
              </div>

              <div>
                <label className="block mb-1">Hình ảnh</label>
                <div className="border border-dashed p-4 rounded flex items-center gap-4">
                  <input type="file" accept="image/*" onChange={handleImageSelect} />
                  {previewFile && <span>Đã chọn ảnh</span>}
                </div>
              </div>

              <div>
                <label className="block mb-1">Mô tả</label>
                <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full border px-3 py-2 rounded" rows={3} />
              </div>

              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 border rounded">Hủy</button>
                <button type="submit" disabled={isUploading} className="px-4 py-2 bg-pink-500 text-white rounded">{isUploading ? "Đang lưu..." : "Lưu"}</button>
              </div>
            </form>
          </div>
        </div>
